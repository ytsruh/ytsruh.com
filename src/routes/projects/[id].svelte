  ...
  <div class="w-full">
    ...
    <div class="flex flex-row py-10">
      <div class="basis-1/2 m-5">
        <img src={`../${project.img}`} class="w-full bg-nav h-36 lg:h-72 object-cover" alt={project.name} />
      </div>
      <div class="basis-1/2 m-5">
        ...
        <div class="my-5 flex flex-row">
          {#if project.link}
            <a
              href={project.link}
              target="_blank"
              class="bg-theme text-white px-4 py-3 mx-2 rounded-md transition">View this project</a
            >
          {/if}
          {#if project.github}
            <a
              href={project.github}
              target="_blank"
              class="bg-theme text-white px-4 py-3 mx-2 rounded-md transition">View on Github</a
            >
          {/if}
        </div>
        <div class="my-5">
